...
const must=['index.html','access.html','member.html','owner.html','status.html','common.js','member.js','owner.js','_worker.js','vercel.json','wrangler.toml','health.json'];
const fail=[];
...
for(const [f,src] of [['member.js',member],['owner.js',owner],['_worker.js',worker]])if(/\?(?:t|ts|timestamp)=\$?\{?Date\.now\(\)/.test(src))fail.push(`${f} contains timestamp cachebuster`);
...
console.log('QA PASS: v52.2 same-origin access gateway contract is internally consistent.');
